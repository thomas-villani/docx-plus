...
from docx_plus.tables import (
    Shading, set_cell_shading, set_row_shading, set_table_shading,
)
...
set_table_shading(table, Shading(fill="F2F2F2"))             # whole-table default
...
`color` is that pattern's foreground and does nothing while `pattern` is
`"clear"`. All three validate at construction, so `fill="red"` or
`"#FF0000"` raises.

!!! note "Rows have no shading element"
    `CT_TrPr` has no `w:shd` child — there is no row-level shading in the
    format. Word implements "shade this row" by writing the same fill into
    every cell, and so does `set_row_shading`. It iterates `<w:tc>`
    elements rather than `Row.cells`, so a cell spanning several columns is
    written once rather than once per column.
...
# A fresh cell holds an empty paragraph and no runs, so add one rather than
# indexing into `.runs` — `paragraphs[0].runs[0]` is an IndexError until
# something has written text.
...
- `merge_cells(start, end)` — the two cells are diagonal corners of a
  rectangular region. Content from every cell moves into the top-left one,
  which is what is returned and is **not necessarily `start`**. A
  non-rectangular (L- or T-shaped) selection raises `InvalidMergeError`
  (`DocxPlusError` + `ValueError`).
- `unmerge_cell(cell)` — works from *any* cell in the region, including a
  vertical continuation. Content stays in the anchor; the restored cells
  are empty, matching Word's "Split Cells". Widths divide evenly, because
  the originals were summed away by the merge. Idempotent on an unmerged
  cell.
...
hands back cells that look separate and are not, and `row.cells` reports a
column count Word never draws. **If a table you did not author behaves
...
Returns `0` on any table Word wrote recently. It refuses by default to drop
text held in a continuation cell — that text is invisible in Word, so
keeping it would surface hidden content and dropping it silently would lose
data. Pass `discard_content=True` to choose.
...
!!! warning "Direct formatting only"
    The table-style cascade (table style → `w:tblStylePr` conditional
    branch → direct `w:tcPr`) is **not** resolved here. A `Table Grid`
    table reads back with `borders == {}` — true of its XML, not of its
    appearance.

    Don't use this to answer "does this table have borders?". Use it to
    answer "what did someone write directly on it?".
...
There is one entry per `<w:tc>`, so a merged cell appears once.
...
set_table_borders(table,
                  all_edges=Border(style="single", size=12, color="2F5496"),
...
- [How table formatting works](../concepts/tables.md) — the two merge
  encodings, row shading, and what the cascade doesn't cover
- [Page borders](layout.md#page-borders) are a different module
- Reference: [`tables.borders`](../reference/tables-borders.md),
  [`tables.shading`](../reference/tables-shading.md),
  [`tables.merge`](../reference/tables-merge.md),
  [`tables.read`](../reference/tables-read.md)
- Example: `table_formatting.py`
